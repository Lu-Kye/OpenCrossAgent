...
REM dev-installer-build.bat
REM Build + npm link oca-installer
REM Supports: Windows (cmd / PowerShell)
REM Usage: scripts\dev-installer-build.bat

set "SCRIPT_DIR=%~dp0"
set "PROJECT_ROOT=%SCRIPT_DIR%.."
set "PKG_DIR=%PROJECT_ROOT%\installer"
set "PKG_NAME=@oca/oca-installer"
...
if !errorlevel! neq 0 (
  ...
  echo    Try manually: cd installer ^&^& npm link
  exit /b 1
)

echo.
echo [OK] %PKG_NAME% built and linked!
echo    You can now run: oca-installer --help
